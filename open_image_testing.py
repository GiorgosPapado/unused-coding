...
def feature_extraction_fn(image_paths, check_image=True):
    ...
    if check_image:
        ...
        processed_images = []
        unique_filenames = list(OrderedDict.fromkeys(image_paths))
        for image_file in unique_filenames: #image_paths:
            with Image.open(image_file, 'r') as img:
                    #img_array = np.asarray(img)
                processed_images.append(img)
                pixel_values = feature_extractor(images=img, return_tensors="pt")["pixel_values"]
                    #image_with_labels = Image.fromarray(np.uint8(np.clip(processed_images, 0, 255)))
    # Use feature_extractor to obtain encoder inputs
        #np.array(processed_images, dtype=np.uint8)
            #encoder_inputs["pixel_values"] = feature_extractor(images=processed_images, return_tensors="pt")["pixel_values"]
            encoder_inputs.append(pixel_values)
        merged_pixel_values = np.concatenate(encoder_inputs, axis=0)
    
    encoder_inputs = feature_extractor(images=images, return_tensors="pt")
    return encoder_inputs.pixel_values
    #return merged_pixel_values
...
# %%
preprocess_fn(new_dataset, 15, check_image = True)
...
# %%
#EVALUATION
preprocess_fn(new_val_dataset, 15, check_image = True)

# %%
#EVALUATION
val_processed_dataset = new_val_dataset.map(
    function=preprocess_fn,
    batched=True,
    fn_kwargs={"max_target_length": 128},
    #remove_columns=new_val_dataset.column_names
)
...
training_args = Seq2SeqTrainingArguments(
    ...
    output_dir="./image-captioning-output-102epochs",
    num_train_epochs= 2
)
...
# %%
trainer.save_model("./image-captioning-output-102epochs")

# %%
tokenizer.save_pretrained("./image-captioning-output-102epochs")

# %%
from transformers import pipeline
# full dataset trained model can be found at https://huggingface.co/nlpconnect/vit-gpt2-image-captioning
image_captioner = pipeline("image-to-text", model="./image-captioning-output-102epochs", max_new_tokens=15)
